...
<body>
<br>
<h2>Data Slip Gaji Pegawai Kontrak
  <div align="right">
		...
		$jbt=mysql_fetch_array($nm2);
		  ?>
			<tr align="left" valign="top">
				...
				<td>
					<?php echo "Rp. ".format_rupiah($jbt[gaji_pokok]); ?>			</td>
				<td>
					<?php echo "Rp. ".format_rupiah($re[gaji_bersih]); ?>			</td>
                <td align="center">
<!--					<a href="?page=u-gaji&id=<?php echo $re[id_slip]; ?>&io=Kontrak">Update</a>	
-->					<a href="?page=slip-K&id=<?php echo $re[id_slip]; ?>" onClick="return confirm('Apakah Anda yakin menghapus data ini ?')">Delete</a>				</td>															
			</tr><?php }
	 $sql=mysql_query("select * from daftar_gaji where status_pgw='Kontrak'"); 
...
<?php
if(isset($_GET[id])){
	$del=mysql_query("delete from daftar_gaji where id_slip='$_GET[id]'");
	echo"<script>window.alert('Berhasil')</script>";
	echo"<meta http-equiv='refresh' content='0;URL=?page=slip-K&ok' />";
}
